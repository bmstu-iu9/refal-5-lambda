...
* `t.DeclarationCommand` — объявление или определение чего-то на C++, сюда же
  ...
  * `#CmdEnum`, `#CmdDefineIndent`, `#CmdEmitNativeCode` — тот же смысл,
    что и для высокоуровневых команд;
  * `#CmdFuncDescr` — генерация описателя функции, содержащего ссылку на код
    и строку имени функции, именно он экспортируется из модуля или является
    локальным, тело самой функции всегда локально;
  * `#CmdExtern` — ссылка на описатель функции из другого модуля;
  * `#CmdSwap` — генерация тела функции статического ящика;
